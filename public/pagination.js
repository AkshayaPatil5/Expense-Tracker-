...
async function setrow(e){
  ...
  try{
    const page =1;
    let res = await axios.get(`http://localhost:3000/expense/products?page=${page}&row=${rowpara}`)
...
async function getProducts(page){
  ...
  let res = await axios.get(`http://localhost:3000/expense/getexpenses?page=${page}&row=${rowpara}`)
  const {products , ...pagedata} = res.data;
  listproduct(res.data.products);
  showpagination(pagedata);
