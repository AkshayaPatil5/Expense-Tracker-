...
document.getElementById('paybutton').onclick = async function (e) {
    ...
    const response = await axios.get('http://localhost:4000/premium/purchasepremium', {
        headers: {
            "Authorization": token
        }
    });
    ...
    var options = {
        ...
        "handler": async function (response) {
            await axios.post('http://localhost:4000/premium/updatetranctionstatus', {
...
function download() {
    const token = localStorage.getItem('token');
    axios.get('http://localhost:4000/premium/download', { headers: { "Authorization": token } })
        .then((response) => {
            console.log(response)
